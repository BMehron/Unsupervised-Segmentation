...
from torchvision.datasets.voc import (DATASET_YEAR_DICT, VisionDataset, os, verify_str_arg)


def _resize_pseudolabel(pseudolabel, img, acseg=False):
    if pseudolabel.shape[:2] != img.shape[:2] and ( acseg or 
...
class VOCSegmentationWithPseudolabelsBase(VisionDataset):
    ...
    def _load(self, index: int, acseg=False):
        # Load image
        ...
        pseudolabel = _resize_pseudolabel(pseudolabel, img, acseg)  # HACK HACK HACK
...
class VOCSegmentationWithPseudolabels(VOCSegmentationWithPseudolabelsBase):
    ...
    def __init__(self, *args, segments_dir, transform = None, label_map = None, acseg = False, **kwargs):
        super().__init__(*args, **kwargs)
        self.acseg = acseg # Add by Mekhron
        self._prepare_segments_dir(segments_dir)
        self.transform = transform
        self._prepare_label_map(label_map)

    def __getitem__(self, index: int):
        img, target, pseudolabel, metadata = self._load(index, self.acseg)
